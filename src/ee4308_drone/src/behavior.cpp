...
namespace ee4308::drone
{
        ...
        : Node(name, options)
    {
        ...
        this->cruise_height_ = ee4308::getParameter<double>(this, "cruise_height", 5.0).as_double();
        ...
        this->state_ = BEGIN;
        this->transition_(TAKEOFF);// ENTRY POINT INTO FINITE-STATE MACHINE LOOP
        this->timer_ = this->create_timer(1s / frequency_, std::bind(&Behavior::callbackTimer_, this)); //  this is where the callbackTimer_function is run, so the plan in question is updated automatically. 
    }
    ...
    void Behavior::callbackTimer_()
    {
        ...
        if (reachedWaypoint_())  // change the 1: if (reached waypoint)
        {
            if (state_ == TAKEOFF)
            {
                transition_(turtle_stop_ ? INITIAL : TURTLE_POSITION);
            }
            else if (state_==INITIAL) {
                transition_(turtle_stop_ ? LANDING : TURTLE_POSITION); // i.e. if the turtle has not stopped, transition it to landing state
            }
        ...
        double dronex = odom_.pose.pose.position.x;
        double droney = odom_.pose.pose.position.y;
        double dronez = odom_.pose.pose.position.z;
        ...
        requestPlan_(dronex,droney,dronez,waypoint_x_,waypoint_y_,waypoint_z_);
    ...
    void Behavior::transition_(int new_state)
    {
        ...
        if (state_== TAKEOFF)
        {
            ...
            setWaypoint_(turtle_plan_.poses.front().pose.position.x,turtle_plan_.poses.front().pose.position.y,cruise_height_);
        }
        else if (state_==LANDING){
            setWaypoint_(initial_x_,initial_y_,initial_z_); // LANDING describes going to the ground position, meaning we can finally draw on initial_z_ for z waypoint
        }
        else if (state_ == END) // ONLY reached when the initial state was already reached and landing was initiated. and once the ncesary start position is reached with LANDING as the current state, the drone turns off, which is the purpose of the END state
        {
            // turn off the robot
            std_msgs::msg::Bool msg_enable;
    ...
    bool Behavior::reachedWaypoint_()
    {
        ...
        return std::sqrt(dx*dx + dy*dy + dz*dz) <= reached_thres_;
    }
